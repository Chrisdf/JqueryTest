...
var app = {
    ...
    onDeviceReady: function() {
        ...
        var deviceLocationElement = document.getElementById('device_location');
            ...
            var doc = db.document('testdoc');

            db.collection('numbers').link('#work_order_list', {
                template: '<ul>{^{for items}}<li>{^{:work_order_no}}: {^{:job_scope}}</li>{{/for}}</ul>'
            }, {
                $wrap: 'items',
                $wrapIn: doc
            });
            ...
            numbers.insert({"work_order_no": "TESTING", "job_scope": "THIS IS A TESTING JOB", "description": "Hello I am a description meep" });
            var endtime = performance.now();
            var total = endtime-starttime;
            console.log("TIME TO INSERT A RECORD: " + (total) + ".");

            var startTime = performance.now();
            var work_order_data = numbers.find({work_order_no: "TESTING"}, {});
            var endTime = performance.now();
            var Total = endTime - startTime;
            console.log("TIME TO FIND A RECORD: " + (Total) + ".");

            console.log(work_order_data[0]["job_scope"]);

            var workOrderList = document.getElementById('work_order_list');


            var bluetoothDevices = bluetoothSerial.list(function(devices) {
